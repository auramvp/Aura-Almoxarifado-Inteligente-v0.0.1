...
import {
    Building2, User as UserIcon, Mail, Lock,
    CheckCircle2, Loader2, ArrowRight, ShieldCheck,
    Building, Sparkles
} from 'lucide-react';
import { db } from '../services/db';

const WarehouseRegistration: React.FC = () => {
    ...
    const handleRegister = async (e: React.FormEvent) => {
        ...
        try {
            ...
            console.error("Erro no registro de unidade:", err);
        ...
        return (
            <div className="min-h-screen bg-gradient-to-br from-blue-900 via-indigo-900 to-slate-900 flex items-center justify-center p-4">
                <div className="bg-white/10 backdrop-blur-xl border border-white/20 p-12 rounded-[40px] shadow-2xl max-w-lg w-full text-center animate-in zoom-in duration-500">
                    ...
                    <h2 className="text-2xl font-black text-white mb-4">Acesso Configurado!</h2>
                    <p className="text-blue-100/80 leading-relaxed mb-6 text-sm">
                        Seu cadastro para gerenciar o almoxarifado <strong>{unitData.name}</strong> foi concluído com sucesso!
                    </p>
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-950 via-blue-900 to-indigo-950 flex items-center justify-center p-4 overflow-hidden relative">
            ...
            <div className="max-w-3xl w-full grid grid-cols-1 lg:grid-cols-2 gap-0 overflow-hidden bg-white/5 backdrop-blur-2xl border border-white/10 rounded-[40px] shadow-2xl animate-in fade-in slide-in-from-bottom-8 duration-700">
                ...
                <div className="p-6 lg:p-10 bg-gradient-to-br from-[#0F172A] to-[#1E293B] flex flex-col justify-between text-white relative">
                    ...
                    <div className="relative z-10">
                        ...
                        <h1 className="text-2xl lg:text-3xl font-black leading-tight mb-4 text-white">
                            Gestão de Unidade: <span className="text-blue-400">{unitData.name || 'Nova Unidade'}</span>
                        </h1>

                        <p className="text-slate-100 text-base font-medium leading-relaxed mb-4">
                            Você foi designado como responsável por este almoxarifado. Configure sua senha para começar a gerenciar o estoque.
                        </p>

                        <div className="space-y-3 mt-8">
                            {[
                                { icon: ShieldCheck, text: "Controle Total de Estoque" },
                                { icon: Building, text: "Dashboard em Tempo Real" },
                                { icon: Sparkles, text: "Alertas de Ruptura com IA" }
                            ].map((item, i) => (
                ...
                <div className="p-6 lg:p-10 flex flex-col justify-center bg-slate-900/40 backdrop-blur-3xl">
                    <div className="mb-6 text-center lg:text-left">
                        ...
                        <p className="text-slate-400 font-bold text-[11px]">Defina seus dados para acessar o sistema.</p>
                    </div>
                    ...
                    <form onSubmit={handleRegister} className="space-y-4">
                        <div className="space-y-2">
                            <label className="text-[9px] font-black text-blue-400 uppercase tracking-[2px] ml-1">E-mail de Responsável</label>
                            <div className="flex items-center gap-3 p-3 bg-white/5 border border-white/10 rounded-2xl">
                                <Mail size={14} className="text-blue-500" />
                                <p className="text-white font-bold text-[11px] truncate">{unitData.email || '---'}</p>
                            </div>
                        </div>

                        <div className="space-y-4 pt-1">
                            <label className="text-[9px] font-black text-blue-400 uppercase tracking-[2px] ml-1">Dados Pessoais</label>
                            ...
                            <div className="relative group">
                                <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                                    <Lock size={16} className="text-slate-500 group-focus-within:text-blue-500 transition-colors" />
                                </div>
                                <input
                                    required
                                    type="password"
