  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <header className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-100">Importação de Dados</h2>
          <p className="text-slate-500 dark:text-slate-400">Migre suas planilhas do Excel ou arquivos XML (NFe) para o sistema Aura em instantes.</p>
        </div>
        {status !== 'idle' && (
           ...
           <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-2xl mx-auto max-h-[400px] overflow-y-auto p-2">
              {availableSources.map((source) => {
                  ...
                  <button 
                    ...
                    className={`flex flex-col items-start p-4 rounded-xl border-2 transition-all group text-left ${
                        isImported 
                  ...
                  >
                     <div className="flex items-center gap-3 w-full mb-2">
                        <div className={`p-2 rounded-lg ${
                            isImported ? 'bg-emerald-200 text-emerald-700' :
