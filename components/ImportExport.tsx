...
import {
  ...
  Settings2, FileSpreadsheet, Trash2, PackageSearch, Clock
} from 'lucide-react';
...
const ImportExport = ({ user }: any) => {
  ...
  const [exportHistory, setExportHistory] = useState<AuditLog[]>([]);

  // Export State
  const [exportType, setExportType] = useState<'products' | 'suppliers' | 'movements' | 'inventory'>('products');
    ...
    try {
      ...
      if (exportType === 'products') {
        ...
        data = prods.map(p => ({
          ...
          'PMED': p.pmed,
          'Local': p.storageLocation,
          'Ativo': p.active ? 'Sim' : 'Não'
        }));
        details = `${data.length} produtos exportados`;
        ...
        details = `${data.length} fornecedores exportados`;
      } else if (exportType === 'movements') {
        const movs = await db.getMovements();
        data = movs.map(m => ({
          ...
          'NF': m.invoiceNumber,
          'Destino/Origem': m.destination
        }));
        details = `${data.length} movimentações exportadas`;
      } else if (exportType === 'inventory') {
        const prods = await db.getProducts();
        const balances = await db.getStockBalances();
        data = balances.map(b => {
          const p = prods.find(production => production.id === b.productId);
          return {
            'Código': p?.cod || 'N/A',
            'Produto': p?.description || 'Desconhecido',
            'Unidade': p?.unit || '',
            'Quantidade em Estoque': b.quantity,
            'Local': b.locationId,
            'Valor Total': (b.quantity || 0) * (p?.pmed || 0)
          };
        });
        details = `${data.length} itens de estoque exportados`;
      }
      ...
      if (user && user.id) {
        await db.addExportLog(user.id, exportType, data.length, details);
        await loadHistory();
      }

      addLog(`Exportação de ${exportType} concluída com sucesso.`);
    } catch (error: any) {
      console.error("Erro na exportação:", error);
      setErrorMessage("Falha ao exportar dados.");
  ...
  return (
            ...
            {status === 'idle' && (
              <div className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <button
                    onClick={() => { setTarget('products'); document.getElementById('file-upload')?.click(); }}
                    className="group relative p-8 bg-blue-600 dark:bg-blue-700 rounded-3xl text-left hover:scale-[1.02] transition-all duration-300 shadow-xl shadow-blue-500/20 overflow-hidden"
                  >
                    ...
                    <div className="relative z-10">
                      <div className="w-14 h-14 bg-white/20 backdrop-blur-sm rounded-2xl flex items-center justify-center mb-6 text-white group-hover:scale-110 transition duration-300">
                        <PackageSearch size={32} />
                      </div>
                      <h3 className="text-2xl font-black text-white mb-2">Cadastro de Itens</h3>
                      <p className="text-blue-100 font-medium">Importar catálogo de produtos</p>
                    </div>
                  </button>

                  <button
                    onClick={() => { setTarget('movements'); document.getElementById('file-upload')?.click(); }}
                    className="group p-8 bg-slate-900 dark:bg-slate-800 rounded-3xl text-left hover:scale-[1.02] transition-all duration-300 border border-slate-800 dark:border-slate-700 hover:border-blue-500/30"
                  >
                    <div className="w-14 h-14 bg-white dark:bg-slate-700 rounded-full flex items-center justify-center mb-6 text-slate-900 dark:text-white group-hover:bg-blue-600 group-hover:text-white transition duration-300">
                      <ArrowRight size={24} />
                    </div>
                    <h3 className="text-2xl font-black text-slate-800 dark:text-white mb-2">Movimentações</h3>
                    <p className="text-slate-500 dark:text-slate-400 font-medium">Entradas e saídas de estoque</p>
                  </button>
    ...
    <div className="flex flex-col lg:flex-row gap-8 items-start animate-in fade-in duration-300">
      <div className="flex-1 w-full bg-white dark:bg-slate-900 p-8 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm">
        <div className="mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
          {[
            { id: 'products', label: 'Catálogo de Produtos', icon: PackageSearch },
            { id: 'suppliers', label: 'Lista de Fornecedores', icon: CheckCircle2 },
            { id: 'movements', label: 'Movimentações de Estoque', icon: ArrowRight },
            { id: 'inventory', label: 'Relatório de Estoque (Saldo)', icon: Table }
          ].map((opt) => (
            <button
              key={opt.id}
              onClick={() => setExportType(opt.id as any)}
              className={`p-4 rounded-xl border-2 text-left transition-all flex items-center gap-4 ${exportType === opt.id
                ? 'border-blue-600 bg-blue-50 dark:bg-blue-900/20 ring-2 ring-blue-500/20'
                : 'border-slate-100 dark:border-slate-800 hover:border-blue-300 dark:hover:border-slate-600'
                }`}
            >
              <div className={`p-3 rounded-full ${exportType === opt.id ? 'bg-blue-600 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-400'}`}>
                <opt.icon size={20} />
              </div>
              <div>
                <h4 className={`font-bold ${exportType === opt.id ? 'text-blue-700 dark:text-blue-400' : 'text-slate-700 dark:text-slate-300'}`}>{opt.label}</h4>
              </div>
            </button>
          ))}
        </div>
        ...
        <button
          ...
          className="w-full py-4 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold text-lg shadow-lg shadow-blue-500/20 disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-3 transition-all transform active:scale-[0.99]"
        >
          {isExporting ? <Loader2 className="animate-spin" /> : <FileUp />}
          {isExporting ? 'Gerando Arquivo...' : 'Exportar Planilha Agora'}
        </button>
      </div>

      {/* Export History Sidebar */}
      <div className="w-full lg:w-96 space-y-6">
        <div className="bg-slate-900 dark:bg-slate-950 text-white p-6 rounded-3xl shadow-xl border border-slate-800 relative overflow-hidden">
          <div className="flex items-center gap-3 mb-6 relative z-10">
            <Clock className="text-emerald-400" size={20} />
            <h3 className="font-bold text-sm uppercase tracking-widest">Histórico de Exportação</h3>
          </div>

          <div className="space-y-4 relative z-10">
            {exportHistory.length > 0 ? exportHistory.slice(0, 5).map((log) => {
              let details: any = {};
              try {
                details = typeof log.afterJson === 'string' ? JSON.parse(log.afterJson) : log.afterJson || {};
              } catch (e) { details = {}; }
              ...
              return (
                <div key={log.id} className="text-xs space-y-1">
                  ...
                  <div className="bg-slate-800/50 p-3 rounded-xl border border-slate-700">
                    <p className="font-bold text-white uppercase">{details.type || 'Exportação'}</p>
                    <p className="text-slate-400 mt-1">{details.details || 'Concluído'}</p>
                  </div>
                </div>
              );
            }) : (
              <p className="text-slate-500 text-xs italic">Nenhuma exportação recente.</p>
            )}
          </div>
        </div>
      </div>
